...
FULL_PATH = "征订记录合并表(3000精简测试版).xlsx"          # 征订目录（全集）
# SEL_PATH = "订购明细-25RT_12_13.xlsx"        # 已订明细
...
# FULL_PATH = "（25RT）征订记录合并表.xlsx"          # 征订目录（全集）
# SEL_PATH = "订购明细-25RT_12_13.xlsx"        # 已订明细
# MODEL_PATH = "book_selector_online.pkl"  # 模型持久化路径
# OUT_PATH = "(25RT)模型选书结果_模拟.xlsx"        # 输出结果



# 参数配置区
CONFIG = {
    "threshold": 0.55,  # 选书门槛（越高越严格）
...
def main():
    # 1. 读取数据 + 打标签
    df = load_data(FULL_PATH, sel_path=SEL_PATH)
    ...
    # 5. 训练 / 更新模型
    clf, model_metrics = train_model_incremental(
        X_all, y, MODEL_PATH, CONFIG["class_weight"]
    )

    # 6. 用当前模型“模拟选书”，并计算命中率 / 查全率
    df_scored, df_rec, rec_metrics = score_and_recommend(
        df, clf, vectorizer, CONFIG["threshold"]
    )

    # 7. 写 Excel
    save_results(df_scored, df_rec, model_metrics, rec_metrics, OUT_PATH)
...
def main_infer_only():
    ...
    df_scored["score_final"] = (
        ...
        + 0.2 * df_scored["rule_pos_score"]
        - 0.1 * df_scored["rule_softneg_score"]
    )
...
if __name__ == "__main__":
    ...
    evaluate_result_vs_order(
        result_path=OUT_PATH_INFER,
        result_sheet="全部书目_含打分与推荐",
        order_path=SEL_PATH,  # 已订明细
        # order_path="订购明细-25LP07.xlsx",
        out_path="模型选书_与订购明细对照评估.xlsx",
    )
